package app.osmosi.heater.model;

public class HotWaterTimer {
  private final int id;
  private final int hours;
  private final int minutes;
  private final int timeout;
  private final int dayTriggered;

  public HotWaterTimer(int id, int hours, int minutes, int timeout, int dayTriggered) {
    this.id = id;
    this.hours = hours;
    this.minutes = minutes;
    this.timeout = timeout;
    this.dayTriggered = dayTriggered;
  }

  public HotWaterTimer withDayTriggered(int dayTriggered) {
    return new HotWaterTimer(this.id, this.hours, this.minutes, this.timeout, dayTriggered);
  }

  public int getId() {
    return id;
  }

  public int getHours() {
    return hours;
  }

  public int getMinutes() {
    return minutes;
  }

  public int getTimeout() {
    return timeout;
  }

  public int getDayTriggered() {
    return dayTriggered;
  }

  public int getTotalMinutes() {
    return (this.getHours() * 60) + this.getMinutes();
  }
}
